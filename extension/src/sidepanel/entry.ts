...
const requestClient = new RequestClient({
  ...
  update: (request) => {
    ...
    if (request.state === "TERMINAL") {
      ...
      if (request.outcome === "FAILED" || request.outcome === "UNKNOWN")
        showFailure(
          typeof request.code === "string" ? request.code : "INTERNAL_FAILURE",
        );
      else
        setStatus(
  ...
  result: (response) => {
    if (
      ...
      const candidates = response.candidates
        ...
        .filter((candidate): candidate is WorkflowCandidate => !!candidate);
  ...
  connection: (delayed) => {
    if (delayed) setStatus("연결 복구 중 · 요청 상태를 확인하고 있습니다.");
  },
});
...
const activityLabel = (stage: ActivityStage): string =>
  ({
    ...
    PAGE_API_PREPARING: "페이지 API 실행 조건을 확인하는 중입니다.",
    PAGE_API_DISPATCH: "페이지 API를 호출하는 중입니다.",
    PAGE_API_RETURNED: "페이지 API 응답을 확인하는 중입니다.",
