...
| `offscreen` | An offscreen document can make HTTP requests immune to user CSP, run local inference, or play audio without an open panel | Used for the localhost LLM proxy, the optional local WebGPU vision worker, tab recording, validated download staging, the local cloud bridge, and successful `/watch /beep` tones. Local vision receives only the screenshot/prompt already selected for the dedicated vision path; Watch audio receives only a style selector, never page content or an arbitrary URL. |
