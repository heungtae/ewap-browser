...
Extension service worker
  └─ Native Messaging (허용 확장 ID, typed request)
      └─ Company Agent Host
          ...
          └─ Windows ACL named pipe + mutual request authentication -> codex-chat-bridge -> AI Hub / local LLM
```

Agent Host는 요청을 bridge에 보낼 때 다음을 수행한다.

1. 관리자 ACL로 보호된 Host/bridge 구성에서 허용된 pipe endpoint와 고정 header 목록을 선택한다.
...
- Host↔bridge는 loopback HTTP를 사용하지 않는다. 설치 단위 named pipe의 DACL은 Host service identity와 bridge service identity만 연결·생성할 수 있게 하고, pipe name은 deployment마다 난수화한다. 연결 뒤 Host와 bridge는 관리자 ACL 구성의 각자 key로 요청 ID, body hash, issued-at, 단발 nonce를 상호 서명하고 60초 nonce cache로 replay를 거부한다.
- Agent Host와 bridge의 설정 파일, static header, pipe endpoint, signing key는 관리자 ACL로 보호하고 로그에 header 값·prompt·response body를 남기지 않는다. 확장 Managed Storage에는 native host 식별자만 둘 수 있다.
- 허용하지 않은 request header는 bridge로 전달하지 않는다. `Authorization`, cookie, proxy credential 등은 기본 거부다.
- TLS를 사용하고 AI Hub certificate validation을 끄지 않는다. bridge는 TCP listener를 열지 않는다.
...
authoritative Business MCP 읽기와 profile-allowlisted `agentic-read` tool은 Host가 전담한다. extension은 verified profile JWS와 허용 `field_id` 또는 profile tool ID만 보내며, Host는 JWS를 다시 검증하고 그 page profile의 `(server_id, tool_id)`만 관리자 ACL MCP Registry의 고정 gateway route로 해석한다. Business MCP assertion은 AI Hub assertion과 audience를 공유하지 않으며, 값의 model/Side Panel 노출은 signed profile과 Host data-owner allowlist가 모두 허용할 때만 가능하다. 상세 schema와 timeout·cancellation은 [13-page-profile-and-business-mcp-contract.md](13-page-profile-and-business-mcp-contract.md)를 따른다.

R2는 다음 JSON Schema 2020-12 메시지 세 개만 사용한다(모두 `schema_version`, `request_id`, `extension_instance_id`, `run_id`를 포함하고 unknown key를 거부한다).

1. `BIND_SESSION` 요청: `tab_context`, `document_epoch`, `expires_at`를 보내고, Host는 현재 Windows session 검증 뒤 `session_binding_id`, `binding_nonce`, `expires_at`을 반환한다.
2. `ISSUE_CONFIRMATION` 요청: `session_binding_id`, `binding_nonce`, `tab_context`, `document_epoch`, `intent_digest`를 보내고, Host는 일회성 `confirmation_id`, `confirmation_nonce`, `expires_at`을 반환한다.
3. `VERIFY_CONFIRMATION` 요청: 위 binding/confirmation 식별자와 `tab_context`, `document_epoch`, `intent_digest`를 모두 다시 보내며, Host는 expiry·현재 Windows session·extension instance·모든 결속 값이 같은지 확인하고 confirmation을 atomic consume한 `CONSUMED`만 반환한다.
