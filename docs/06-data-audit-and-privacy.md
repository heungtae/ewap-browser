...
| 위치                      | 저장 대상                                                                                                                                                                   | 저장 금지                                                                                                                 |
| ------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| `chrome.storage.local`    | plugin manifest, provider 설정, API key, 정적 header, site permission, UI preference, redacted run summary, 사용자 저장 workflow의 선언형 순서·scope·fingerprint·메타데이터 | executable plugin code, raw page value, password/OTP, action value, workflow 원문 HTML/JS·selector/ref, 모델 tool mapping |
| `chrome.storage.session`  | CDP attach ownership의 tab/run/action과 `attaching`/`attached` phase, [19번 문서](19-tab-scoped-chat-session-design.md)의 redacted·bounded Chat Session/thread context, 5분 이내 workflow 후보·선택 계획(선언형 target 이름/역할·scope·redacted prompt) | target token, ref/node ID, 좌표/box, action value, raw page content와 image, confirmation nonce·실행 대상 ref·코드 원문 |
