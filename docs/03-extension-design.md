...
content script는 document start에 `document_epoch`를 한 번 생성해 `DOCUMENT_REGISTER` 메시지로 service worker에 등록한다. service worker는 sender tab/frame과 등록된 epoch를 묶고, 등록 전 start 요청, 이전 epoch의 재등록, navigation 뒤 늦은 snapshot을 거부한다. worker 재시작 뒤 content script가 같은 현재 epoch를 재등록하기 전에는 run을 시작하지 않는다. run coordinator는 epoch를 생성하거나 교체하지 않는다.
...
run-bound 메시지는 `kind`, `schema_version`, `run_id`, `tab_id`, `frame_id`, `document_epoch`, typed payload를 포함한다. 허용 종류는 `START_PREVIEW`, `START_ASK`, `START_ACT`, `SUBMIT_ACTION_VALUE`, `CONFIRM`, `CANCEL`, `CONTENT_SNAPSHOT`, `EXECUTE_ACTION`, `VERIFY_RESULT`, `PANEL_STATE`, `NATIVE_LLM_REQUEST`로 한정한다. 별도 초기 등록 메시지 `DOCUMENT_REGISTER`는 `run_id` 없이 content script만 보낼 수 있으며 sender tab/frame, document epoch, registration nonce를 검증한다. sender context·tab·frame·등록 epoch·run 상태가 맞지 않으면 거부한다.
...
- `fingerprint_alg`는 현재 `semantic-projection-fp-v1`만 허용한다. exact canonical schema, top-frame node filtering/order, 현재 field/UI 상태를 제외한 state capability, origin class, label category 정규화와 alias 표, RFC 8785 직렬화, SHA-256 unpadded base64url 및 golden vector는 [14-semantic-projection-fingerprint.md](14-semantic-projection-fingerprint.md)를 따른다. raw label, value, URL path/query, ref/model ref는 입력에 없다. 알 수 없는 알고리즘 또는 mismatch는 profile을 거부하고 진행 중 action을 취소한다.
- extension의 빠른 cache는 비권한적이며, replay high-water mark는 Native Host의 관리자 ACL·OS 보호 durable store에 `(deployment_id, profile_id, matcher, fingerprint_alg, fingerprint)`별로 저장한다. 서명·matcher·fingerprint 검증 뒤 Host가 새 version만 atomic compare-and-set으로 수락한다. 낮거나 같은 version, 손상/rollback된 high-water mark는 거부한다. 초기 enrollment 뒤 store가 없어지거나 손상되면 운영 재동기화 전 `PROFILE_UNAVAILABLE`로 fail closed 한다.
- 새 profile과 durable high-water mark가 모두 성공한 뒤에만 extension cache를 atomic swap 한다. resolver transport 오류에는 동일 tab/document/origin/matcher/fingerprint/version의 서명 검증·미만료 cache만 사용할 수 있다. cache miss·만료·resolver의 invalid response·cache corruption 시 old profile을 사용하는 action, pending confirmation, ref registry를 즉시 취소한다.
