...
5. 최대 tool turn을 넘기지 않고 최종 자연어 답변만 Side Panel에 렌더링한다. 개발자 콘솔에는 각 turn의 최종 LLM `messages`/tool schema와 정규화된 응답 message를 기록하며 provider credential·browser credential·raw ref mapping은 기록하지 않는다.
