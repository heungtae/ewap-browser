...
5. 최대 tool turn을 넘기지 않고 최종 자연어 답변만 Side Panel에 렌더링한다. 개발자 콘솔에는 redacted projection, 사용자 질문, tool 이름과 모델 답변만 기록하며 provider credential·browser credential·raw ref mapping은 기록하지 않는다.
