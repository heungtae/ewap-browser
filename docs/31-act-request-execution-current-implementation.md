...
sequenceDiagram
    ...
    SW->>Provider: Proposed 5.2 LLM intent-classification turn, action tool 없음
    Provider-->>SW: closed route enum
    alt QUESTION 또는 정보 부족
        SW->>Provider: 정보성 turn, action proposal tool 없음
        Provider-->>Panel: answer text and VERIFIED terminal
    else ANALYSIS_READ_REQUIRED
        SW->>SW: Proposed 5.3~5.6 source 발견·read·정규화
        SW->>Provider: projection + bounded analysis context
        Provider-->>Panel: analysis answer and VERIFIED terminal
    else ACTION_REQUIRED
        SW->>SW: Profile resolve, 6 도구 발견, 7 workflow candidate discovery
...
| 6. 도구 발견                             | `extension/src/service-worker/page-derived-actions.ts`의 `pageDerivedActionTools`, `selectActActionTools`                                                                                           | **목표:** 5.2가 `ACTION_REQUIRED`일 때만 현재 snapshot의 visible/enabled control에서 action tool을 구성한다. **현재:** 별도 5.2 gate가 없어 모든 Act request에서 이 계산을 수행한다.                                                                                                                                                   | Implemented, Proposed route gate |
| 7. workflow 분기                         | `act-chat-start.ts`의 workflow candidate 분기, `workflow-selection-message-handler.ts`, `workflow-start-message-handler.ts`, `workflow-session-actions.ts`                                          | **목표:** `ACTION_REQUIRED`일 때만 후보를 표시·선택·계획 확인한다. **현재:** 별도 5.2 gate가 없어 후보가 있으면 정보성 질의도 Provider 전에 `WORKFLOW_CANDIDATES`에서 대기한다. 선택/시작 직전 tab, origin, path, document epoch를 다시 확인한다.                                                                                      | Implemented, Proposed route gate |
...
### 3.1 Proposed 5.2 요청 처리 경로 결정 계약

5.2는 사용자가 선택한 `mode=act`를 `ask`로 바꾸는 단계가 아니다. Act 안에서 후속 capability와 workflow 탐색 여부를 제한하는 fail-closed route 결정이다. Proposed `ask-act-intent-router.ts`는 현재 문서에 없는 신규 연결 지점이며, **LLM에 별도 intent-classification turn을 요청**한다.
...
5.2는 사용자에게 답변을 생성하는 turn이 아니라 **방향만 정하는 LLM turn**이다. `QUESTION`과 `ANALYSIS_READ_REQUIRED`는 이후 8단계의 읽기 전용 Provider turn에서 답변을 만들고, `ACTION_REQUIRED`만 action tool과 workflow 후보를 준비한다.

| route                    | 뒤따르는 경로                                                                                                    | action·사용자 확인                                          |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- |
| `QUESTION`               | 6·7을 건너뛰고 8의 정보성 Provider turn으로 간다. 현재 projection으로 답하거나 정보 부족을 설명한다.             | action tool, workflow 후보, review card, 사용자 확인 없음   |
| `ANALYSIS_READ_REQUIRED` | 5.3~5.6의 R0 분석 data 수집 뒤 8의 분석 Provider turn으로 간다.                                                  | 수집 capability만 별도 검사하며 mutation approval 없음      |
| `ACTION_REQUIRED`        | analysis가 선행 조건이면 먼저 5.3~5.6을 수행한 뒤 6 도구 발견, 7 workflow 분기, 8 action Provider turn으로 간다. | action proposal이 생긴 뒤에만 기존 9~14 승인·실행 경로 사용 |
...
- 29번 Page API Discovery도 수동 redacted hint scan이며, 현재 Ask/Act Provider run의 분석 source 또는 data read로 연결되지 않는다. 28번 collection과 reviewed read-only Page API adapter를 5.3~5.6의 공통 분석 수집 단계로 연결하는 목표 계약은 [32번](32-ask-act-analysis-data-acquisition-design.md)에 정의한다.
- 현재 구현에는 5.2 요청 처리 경로 gate가 없다. 따라서 workflow 후보가 있는 Act 질문은 8단계의 tool-less 답변 판단 전에 `WORKFLOW_CANDIDATES`를 표시할 수 있다. Proposed 5.2가 구현되기 전까지는 이 UX/순서 제약이 남는다.
