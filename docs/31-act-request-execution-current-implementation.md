...
2. `createActChatStart()`가 초기 semantic snapshot과 Profile을 준비한다(3절 단계 2.1~2.2). **현재는** 요청 처리 경로를 먼저 판별하는 gate가 없으므로, 정보성 Act 요청도 action tool 계산을 수행하고 workflow 후보가 있으면 Provider 호출 전에 사용자 선택을 기다릴 수 있다.
3. **목표 경로에서는** 단계 3.1이 `QUESTION`, `ANALYSIS_READ_REQUIRED`, `ACTION_REQUIRED`를 결정한다. `QUESTION`은 Act 모드를 유지한 채 33번과 동일한 단계 5.1~5.7 읽기 전용 답변 흐름을 사용한다. `ANALYSIS_READ_REQUIRED`는 단계 4.1~4.4 뒤 단계 5.1~5.7로 간다. `ACTION_REQUIRED`만 선택적 단계 4.1~4.4와 단계 6.1~7.5.1을 사용한다.
...
3절의 단계 3.1(route 결정), 단계 4.1~4.4(분석 source 발견·선택·권한·read·정규화), Act용 단계 5.1~5.7 연결은 **Proposed, not implemented**다. 따라서 현재 Act 요청은 세 route로 분기하지 않고, 33번의 읽기 전용 답변 실행기를 재사용하지 않으며, collection·Page API 분석 데이터를 Provider turn에 연결하지 않는다.
...
sequenceDiagram
    ...
    SW->>Provider: Proposed 3.1 LLM intent-classification turn, action tool 없음
    ...
    alt QUESTION 또는 정보 부족
        ...
        SW->>SW: Proposed 4.1~4.4 source 발견·read·정규화
...
| 3.1 요청 처리 route 결정                  | **Proposed:** `ask-act-intent-router.ts`의 `classifyWithProvider`, `validateRoute`                                                                                                                  | Browser가 initial projection의 read-only 문맥과 prompt를 별도 LLM intent-classification turn에 보낸다. LLM은 `QUESTION`, `ANALYSIS_READ_REQUIRED`, `ACTION_REQUIRED` 중 하나의 closed route만 반환하며, 이 turn에는 action tool·workflow candidate·target·approval input이 없다. Worker가 schema를 검증한 뒤에만 다음 경로를 선택한다. | Proposed, not implemented                |
| 4.1 분석 source 발견                      | **Proposed:** `analysis-data-acquisition.ts`의 `discoverSources`; 재사용 후보: `collection-read-message-handler.ts`의 `handleCollectionDiscover`, `page-api-discovery-domain-handler.ts`의 `handle` | 단계 3.1이 `ANALYSIS_READ_REQUIRED`이거나 analysis 선행이 필요한 `ACTION_REQUIRED`일 때만, Browser dispatcher가 Provider 실행 전에 collection descriptor와 Page API source availability를 함께 발견한다. raw Page API candidate는 Provider에 전달하지 않는다.                                                                          | Proposed, not implemented                |
| 4.2 source 선택·R0 권한                   | **Proposed:** `analysis-data-acquisition.ts`의 `selectSource`, `authorizeRead`; 재사용 후보: `collection-read-message-handler.ts`의 `handleCollectionReadStart`                                     | unique collection은 요청의 전체 데이터 분석 의도에 따라 선택할 수 있고, 복수 source만 Panel 선택을 요구한다. `collection_read` 또는 새 `page_api_read` R0 capability를 action approval과 분리해 검사한다.                                                                                                                              | Proposed, not implemented                |
| 4.3 bounded analysis data read            | **Proposed:** `analysis-data-acquisition.ts`의 `readSelectedSource`, `page-api-read-runner.ts`의 `read`; 재사용 후보: `collection-read-orchestrator.ts`의 `CollectionReadOrchestrator.start`        | collection은 object-specific reader로, Page API는 exact origin/path/version에 묶인 reviewed read-only adapter로만 읽는다. timeout/Stop/scope 변경 뒤 자동 재시도하지 않으며, 29번 discovery candidate 자체는 호출하지 않는다.                                                                                                          | Proposed, not implemented                |
| 4.4 결과 정규화·Provider 재투입           | **Proposed:** `analysis-data-acquisition.ts`의 `buildAnalysisDataContext`; 연결 대상: `ask-chat-runner.ts`, `act-step-runner.ts`                                                                    | bounded sanitized record chunk, coverage, reason, count, evidence만 현재 request 메모리에 둔 뒤 같은 run의 다음 Provider turn에 untrusted context로 전달한다. raw row ID/cursor/selector/function path/endpoint/source는 전달·저장하지 않는다.                                                                                         | Proposed, not implemented                |
| 5.1 읽기 전용 run·projection 준비         | **Proposed Act 연결:** 33번 단계 5.1의 공통 read-only answer runner                                                                                                                                 | `mode=act`와 기존 request owner를 유지한다. 단계 2.2에서 수집하고 단계 3.1에 사용한 initial projection을 run-scoped `model_ref`로 바꾸며 action authority를 만들지 않는다. scope가 달라졌으면 새 page에 자동 재결속하지 않고 `PAGE_SCOPE_STALE`로 끝낸다.                                                                              | Ask implemented, Act connection proposed |
| 5.2 Profile·read binding 해석             | **Proposed Act 연결:** 33번 단계 5.2                                                                                                                                                                | matching Profile 문맥과 approved read-only Business MCP binding만 추가한다. Profile 실패는 일반 읽기 답변을 막지 않는다.                                                                                                                                                                                                               | Ask implemented, Act connection proposed |
| 5.3 읽기 전용 Provider turn               | **Proposed Act 연결:** 33번 단계 5.3                                                                                                                                                                | system prompt, tab thread, projection, 선택적 분석 context와 질문을 보내고 Ask read tool만 제공한다. action proposal tool과 workflow 후보는 제공하지 않는다.                                                                                                                                                                           | Ask implemented, Act connection proposed |
| 5.4 read tool 검증·실행                   | **Proposed Act 연결:** 33번 단계 5.4                                                                                                                                                                | semantic/page/text/find, 조건부 vision·tab context, Profile-bound Business MCP read만 closed schema로 실행한다.                                                                                                                                                                                                                        | Ask implemented, Act connection proposed |
| 5.5 tool 결과 재투입                      | **Proposed Act 연결:** 33번 단계 5.5                                                                                                                                                                | `[UNTRUSTED_TOOL_RESULT]`를 같은 읽기 전용 대화에 넣고 최대 세 Provider turn 안에서 답변을 생성한다.                                                                                                                                                                                                                                   | Ask implemented, Act connection proposed |
| 5.6 답변 완료                             | **Proposed Act 연결:** 33번 단계 5.6                                                                                                                                                                | non-empty 답변을 `VERIFIED`로 끝내며 Act proposal·review·dispatch로 이어가지 않는다.                                                                                                                                                                                                                                                   | Ask implemented, Act connection proposed |
| 5.7 실패·취소·시간 초과                   | **Proposed Act 연결:** 33번 단계 5.7                                                                                                                                                                | 빈 답변, turn 한도, provider 오류, 취소, deadline, scope stale을 dispatch 없이 종료한다.                                                                                                                                                                                                                                               | Ask implemented, Act connection proposed |
| 6.1 action 도구 발견                      | `extension/src/service-worker/page-derived-actions.ts`의 `pageDerivedActionTools`, `selectActActionTools`                                                                                           | **목표:** 단계 3.1이 `ACTION_REQUIRED`일 때만 현재 snapshot의 visible/enabled control에서 action tool을 구성한다. **현재:** 별도 route gate가 없어 모든 Act request에서 이 계산을 수행한다.                                                                                                                                            | Implemented, Proposed route gate         |
| 6.2 workflow 분기                         | `act-chat-start.ts`의 workflow candidate 분기, `workflow-selection-message-handler.ts`, `workflow-start-message-handler.ts`, `workflow-session-actions.ts`                                          | **목표:** `ACTION_REQUIRED`일 때만 후보를 표시·선택·계획 확인한다. **현재:** 별도 단계 3.1 gate가 없어 후보가 있으면 정보성 질의도 Provider 전에 `WORKFLOW_CANDIDATES`에서 대기한다. 선택/시작 직전 tab, origin, path, document epoch를 다시 확인한다.                                                                                 | Implemented, Proposed route gate         |
...
### 3.1 Proposed route 결정 계약 — 실행 단계 3.1

단계 3.1은 사용자가 선택한 `mode=act`를 `ask`로 바꾸는 단계가 아니다. Act 안에서 후속 capability와 workflow 탐색 여부를 제한하는 fail-closed route 결정이다. Proposed `ask-act-intent-router.ts`는 현재 문서에 없는 신규 연결 지점이며, **LLM에 별도 intent-classification turn을 요청**한다.
...
- 현재 구현에는 단계 3.1 요청 처리 route gate와 Act용 단계 5.1~5.7 연결이 없다. 따라서 workflow 후보가 있는 Act 질문은 읽기 전용 답변 판단 전에 `WORKFLOW_CANDIDATES`를 표시할 수 있다. 단계 3.1과 공통 read-only answer runner가 구현되기 전까지는 이 UX/순서 제약이 남는다.
...
Proposed 단계 3.1이 `QUESTION` 또는 `ANALYSIS_READ_REQUIRED`를 반환하면 action용 `runStep`으로 가지 않는다. 이 경우 단계 5.1은 단계 2.2에서 수집하고 route 결정에 사용한 initial projection을 33번의 읽기 전용 흐름과 같이 사용한다. tab/document/page scope가 달라졌으면 fresh action snapshot으로 대체하거나 새 page에 자동 재결속하지 않고 `PAGE_SCOPE_STALE`로 끝낸다.
