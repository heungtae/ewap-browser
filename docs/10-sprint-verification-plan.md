...
- unit: content-owned document registration/epoch, redaction, runtime message schema, sender validation, page-read origin gate, preview mutation deny.
- fixture/E2E: 실제 Chrome semantic projection과 `ref_id` 탐색, document-start registration, navigation 후 stale ref 폐기, worker restart 뒤 registration 전 run 거부, Side Panel preview rendering.
- security negative: blocked origin/localhost/IP/file URL에서 preview snapshot, page `postMessage`, unknown runtime `kind`, registration 전 start, raw input value가 포함된 snapshot을 모두 거부한다. Host/LLM/resolver 호출 또는 mock 성공은 존재하지 않음을 검사한다.

## 4. S2 검증

- unit: exact origin matching, unknown profile deny, R1 policy, sensitive-field deny, value slot/digest canonicalization·TTL·atomic consume·sender/binding validation, audit allowlist serializer.
...
- fixture/E2E: R2 click/key의 confirmation, cross-session/cross-tab/cross-epoch confirmation rejection, navigation 중 확인 무효화, Stop, content disconnect, verifier failed/unknown과 trusted-input-required target 거부를 검사한다.
- security negative: R2 no-confirmation, session-binding/nonce/digest mismatch, capability 없는 synthetic activation, R3 request, `UNKNOWN` 재시도, worker restart 뒤 mutation 복구가 모두 거부된다.

## 6. S4 검증

- unit/contract: Native Messaging framing, allowed extension ID, model snapshot/proposal의 `model_ref` allowlist와 raw-ref/mapping reject, malformed payload, `BIND_SESSION`/`ISSUE_CONFIRMATION`/`VERIFY_CONFIRMATION` schema, cancellation, header allowlist, error mapping.
- local integration: mock SSO broker와 ACL named-pipe mock bridge로 assertion/session-binding success/failure, timeout, cancellation, response-size 제한을 재현한다.
...
- unit/contract: invalid signature/key ID/size, resolver request의 path/query/header data minimization, [14의 `semantic-projection-fp-v1`](14-semantic-projection-fingerprint.md) 665-byte golden JSON/hash, key insertion order 독립성, label alias와 `other`/`none`, state capability, relation ordinal, 현재 field/UI state와 forbidden-field 비영향 및 canonicalization mismatch, same page/different record의 같은 profile match, expiry/future-issued/replay profile, valid cache fallback과 cache invalidation, durable store corruption, SPA major semantic 변화의 profile re-resolution/tool revocation, unknown page Ask basic-read-only/Act deny, activation capability/trusted-input deny, profile 밖 field·server/tool·arbitrary MCP route/header·AI Hub assertion reuse 거부, deterministic binding과 agentic-read exposure isolation, MCP visibility/audit leakage, timeout/late response 및 MCP failure fail-closed, managed policy schema/version/header-key failure.
